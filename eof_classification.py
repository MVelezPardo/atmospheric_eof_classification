...
EOF_GLOBS     = {n: os.path.join(MODEL_EOF_DIR, f"EOF{n}_psl_JFM_cmip6_*.nc") for n in EOF_NUMS}
...
SCA_CTRL_FILE = "data/SCA_psl_EOF2_JFM_obs_1969-2012.nc" # Path to Reanalysis Scandinavian Pattern NetCDF file (20CR-VR recommended)
...
CONSENSUS_WEAK_THRESHOLD   = 0.3             # consensus confidence >= this => weak; below => very weak
...
def _model_label(path):
    ...
    return base[len(prefix):-len(suffix)] if base.startswith(prefix) and base.endswith(suffix) else base

def _rename_latlon(da):
    rmap = {}
    for c in ("latitude", "nav_lat", "y"):
        if c in da.dims and "lat" not in da.dims: rmap[c] = "lat"
    for c in ("longitude", "nav_lon", "x"):
        if c in da.dims and "lon" not in da.dims: rmap[c] = "lon"
    if rmap: da = da.rename(rmap)
    for dim in ("lat", "lon"):
...
def _read_da(path, var=VAR_NAME):
    ...
    if "mode" in da.dims: da = da.isel(mode=0)
    for d in list(da.dims):
        if da.sizes.get(d, 2) == 1: da = da.isel({d: 0})
...
def _wnorm(a, w):     return float(np.sqrt(_wip(a, a, w)))
def _wcor(a, b, w):
...
def _regrid_align(eofs_dict, *ctrls):
    ...
    w2d = _coslat_w2d(lat, lon.size) if USE_COSLAT_WEIGHTS else np.ones((lat.size, lon.size))
    # Split back: EOF2 is aligned[0], EOF3=aligned[1], EOF4=aligned[2], then ctrls
    eof_arrs = {EOF_NUMS[0]: aligned[0], EOF_NUMS[1]: aligned[1], EOF_NUMS[2]: aligned[2]}
...
def _gauss_kernel(sigma_pts):
    if sigma_pts is None or sigma_pts <= 0: return np.array([1.0])
    r = int(np.ceil(3.0*sigma_pts))
    x = np.arange(-r, r+1, dtype=float)
    k = np.exp(-0.5*(x/sigma_pts)**2); k /= k.sum()
    return k

def _conv_rows_periodic(num, den, k):
    if k.size == 1: return num, den
    p = k.size//2
    on = np.empty_like(num); od = np.empty_like(den)
...
def _shift2d(a, dlat, dlon):
    ...
    if dlat > 0: out2[dlat:] = out[:-dlat]
    else:        out2[:dlat] = out[-dlat:]
    return out2
...
def _make_shifts(lat, lon, lat_max, lon_max):
    ...
    dlat = max(dlat, 1e-9); dlon = max(dlon, 1e-9)
...
def _weighted_lstsq(e2, e3, e4, y, w, ridge=0.0):
    ...
    yhat = Xn @ c
    na = np.sqrt(np.sum(w*yhat**2)); nb = np.sqrt(np.sum(w*y**2))
...
def run_subspace(EA_ctrl, SCA_ctrl, triplets):
    ...
    for model, eofs in triplets:
        try:
            eof_das, (EA_a, SC_a), lat, lon, w2d = _regrid_align(eofs, EA_ctrl, SCA_ctrl)
        ...
        lat_sh, lon_sh, _, _ = _make_shifts(lat, lon, SHIFT_LAT_MAX_DEG, SHIFT_LON_MAX_DEG)
        ...
        for i, n in enumerate(EOF_NUMS):
            ea_share = abs(c_EA[i]) / max(np.sum(np.abs(c_EA)), 1e-30) if c_EA is not None else 0.0
            sca_share = abs(c_SCA[i]) / max(np.sum(np.abs(c_SCA)), 1e-30) if c_SCA is not None else 0.0
...
def _test2_favors(da):
    ...
    if (GRL > 0) and (IRL > 0) and (SCN < 1.0): return "EA"
    if (GRL < 0) and (IRL < 0) and (SCN > 1.0): return "EA"
    if (GRL < 1.0) and (IRL > 0) and (SCN > 0):  return "SCA"
    if (GRL > 1.0) and (IRL < 0) and (SCN < 0):  return "SCA"
    return None
...
def _geo_favors(eof_da, rEA, rSCA):
    ...
    t2 = "EA" if (t2ea == "EA" and t2sca != "SCA") else ("SCA" if (t2sca == "SCA" and t2ea != "EA") else None)
    t3ea  = _test3_favors(eof_da * sEA)
    t3sca = _test3_favors(eof_da * sSCA)
    t3 = "EA" if (t3ea == "EA" and t3sca != "SCA") else ("SCA" if (t3sca == "SCA" and t3ea != "EA") else None)
    return t2, t3
...
def run_correlations(EA_ctrl, SCA_ctrl, triplets):
    ...
    for model, eofs in triplets:
        ...
        for n in EOF_NUMS:
            ...
            sgn = 1.0
            if target == "EA" and rEA < 0: sgn = -1.0
            if target == "SCA" and rSCA < 0: sgn = -1.0
...
def _prepare_eof_sample(eof_da, EA_sub, SCA_sub, w2d, mask_flat, w_sqrt):
    ...
    sgn = (1.0 if rEA >= 0 else -1.0) if abs(rEA) >= abs(rSCA) else (1.0 if rSCA >= 0 else -1.0)
    arr = arr * sgn
...
def _load_centers(filepath):
    ...
    if data["reanalysis"] != CTRL_REANALYSIS:
        print(f"[WARN][kmeans] Centers were trained on {data['reanalysis']} "
              f"but current CTRL_REANALYSIS is {CTRL_REANALYSIS}.")
    if data.get("cmip_tag", "") != CMIP_TAG:
        print(f"[WARN][kmeans] Centers were trained on {data.get('cmip_tag','?')} "
    ...
    print(f"[kmeans] loaded centers from {filepath} "
...
def _train_kmeans(EA_ctrl, SCA_ctrl, triplets, centers_file=None):
    ...
    for i, m in enumerate(meta):
        ...
        if ea_like > sca_like:
            ...
            label, conf = "NA", 0.0
        results.setdefault(m["model"], {})[m["eof"]] = {"label": label, "confidence": round(conf, 3)}
    return results
...
def _apply_kmeans(EA_ctrl, SCA_ctrl, triplets, centers_file):
    ...
    """
    centers, cluster_types, saved_lat, saved_lon, mask_flat, w_sqrt = _load_centers(centers_file)
    ...
    for model, eofs in triplets:
        ...
        for n in EOF_NUMS:
            try:
                sample = _prepare_eof_sample(eofs[n], EA_sub, SCA_sub, w2d, mask_flat, w_sqrt)
...
def _consensus(method_results):
    ...
    valid  = [(l, c) for l, c in zip(labels, confs) if l not in ("NA", None)]
    ...
    agreeing = [c for l, c in zip(labels, confs) if l == top_label]
...
def write_outputs(res_sub, res_corr, res_km, output_root=None):
    ...
    # -- Consensus table --
    header = ["Model", "EOF",
              "Subspace_label", "Subspace_conf",
              "Correlation_label", "Correlation_conf",
              "Kmeans_label", "Kmeans_conf",
              "Consensus_label", "Consensus_conf", "Quality", "Warning"]
    rows = []
    for m in models:
        for n in EOF_NUMS:
            ...
            rows.append([m, f"EOF{n}",
                         s["label"], f"{s['confidence']:.2f}",
                         c["label"], f"{c['confidence']:.2f}",
                         k["label"], f"{k['confidence']:.2f}",
                         cons, f"{cons_conf:.2f}", quality, warn])
    ...
    with open(out_txt, "w") as g:
        g.write(NOTE + "\n\n")
        g.write("  ".join(str(header[i]).ljust(widths[i]) for i in range(len(header))) + "\n")
        g.write("  ".join("-"*widths[i] for i in range(len(header))) + "\n")
        for r in rows:
            g.write("  ".join(str(r[i]).ljust(widths[i]) for i in range(len(header))) + "\n")
